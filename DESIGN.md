...
このファイルはClaudeへの指示を兼ねる**デザインの正本**です。
`index.html`（`<style>` タグ内）を変更するとき、このファイルに定義された値・ルールを優先します。
ローカルの `apps/DESIGN.md` と差異がある場合は、より新しい方を正本として扱い、両者を同期させてください。
...
| `--warm` | `#a46d3d` | プライマリアクセント（CTA・ラベル・ドット） |
| `--warm-soft` | `#f4e8d8` | warm の薄い背景・バッジ背景 |
| `--olive` | `#67715d` | service-card 3枚目のトップボーダー |
| `--olive-soft` | `#eaede6` | olive 系の薄い背景 |
| `--brick` | `#8a6158` | nav-cta hover・service-card 4枚目トップボーダー |
...
| `max-width: 680px` | カード2列 → 1列（concerns / services / pricing） |
...
### service-card のトップボーダー（4枚セット）

```css
position: relative; overflow: hidden;
/* ::before で上部 3px のカラーバー */
...
現在の順序: hero(gradient) → concerns(warm) → services(surface) → process(warm) → pricing(surface) → faq(warm) → profile(surface) → contact(warm)
...
- ✅ warm アクセント（ドット・バッジ・ボーダー強調）は `--warm` を使う
...
- このファイルはローカルの `apps/DESIGN.md` のマスターコピーとして扱い、変更後は両者を同期させる
