...
| トークン | 値 | 用途 |
|---|---|---|
| `--accent` | `#52687d` | リンク・セカンダリアクション |
| `--accent-hover` | `#3e5366` | リンク hover・btn-primary hover 背景 |
| `--accent-soft` | `#e7edf2` | アクセント系の薄い背景 |
| `--warm` | `#a46d3d` | プライマリアクセント（**装飾専用**: ドット・ボーダー・カラーバー・入力フォーカス枠・OGP）。**テキストには使わない** |
| `--warm-text` | `#8b5c34` | **暖色のテキスト専用**（section-label / hero-label / case-industry / pricing-card-label / required-pill / 番号サークル）。`--warm` は小さい文字で WCAG AA 未達（3.6〜4.3:1）のため、色相・彩度を保ち明度だけ下げた文字用トークン。全 warm 背景・surface 背景で 4.66:1 以上を確保 |
| `--warm-soft` | `#f4e8d8` | warm の薄い背景・バッジ背景 |
| `--olive` | `#67715d` | case-card / pricing-card 3枚目のトップボーダー |
| `--olive-soft` | `#eaede6` | olive 系の薄い背景 |
| `--brick` | `#8a6158` | nav-cta hover・カラーバー4枚目（現在は未使用枠） |
| `--brick-soft` | `#f1e8e5` | brick 系の薄い背景 |
...
### カラーバー付きカード（case-card / pricing-card）

```css
position: relative; overflow: hidden;
/* ::before で上部 3px のカラーバー。並び順は accent → warm → olive → brick */
::before { content: ""; position: absolute; top: 0; left: 0; right: 0; height: 3px; }
:nth-child(1)::before { background: var(--accent); }
:nth-child(2)::before { background: var(--warm); }
:nth-child(3)::before { background: var(--olive); }
:nth-child(4)::before { background: var(--brick); }
```

### case-card（導入事例カード）

```css
/* カラーバー付きカードに以下を組み合わせる */
.case-industry { /* badge-pill と同形。font-size 11px; color: var(--warm); background: var(--warm-soft); border-radius: 999px; */ }
.case-metric {
  font-family: var(--font-display);
...
現在の順序: hero(gradient) → concerns(warm) → cases(surface) → process(warm) → pricing(surface) → faq(warm) → profile(surface) → contact(warm)
...
- ✅ warm の**装飾**（ドット・カラーバー・ボーダー強調・フォーカス枠）は `--warm` を使う
